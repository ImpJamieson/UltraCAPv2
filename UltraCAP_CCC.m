...
tEnd = 12.65; % simulation end time, equal to maximum permissible charge duration [s]
tSteps = tEnd*100; % number of intervals for time to be computed at
...
D_prime = sigma*k_B*T/e*(1-epsilon)^(1/2); % diffusivity paramter collection of coefficients
Lambda = sigma*tStep/varepsilon*(1-epsilon)^(1/2); % dimensionless number
Upsilon = sigma*k_B*T/e*Gamma*(1-epsilon)^(1/2); % collection of values
Theta = sigma*k_B*T/e/var_rho*Gamma*(1-epsilon)^(1/2); % collection of values
...
[rho_vec_Current,rho_net_vec_Current] = rhoICs_CCC(var_rho,K,sigma,kappa_0,V_start,epsilon,lsPoints); % creates rho initial conditions
[c_vec_Current,c_net_vec_Current] = cICs_CCC(epsilon,c_0,rho_0,D_prime,D_0,F,rho_net_vec_Current,PllPoints,PlsPoints); % creates c initial conditions
c_vec_Initial = c_vec_Current;
c_net_vec_Initial = c_net_vec_Current;
[phi_s_vec_Current] = phi_sICs_CCC(rho_net_vec_Current,K,sigma,kappa_0,epsilon); % creates phi_s initial conditions
[phi_l_vec_Current] = phi_lICs_CCC(phi_s_vec_Current,kappa_0,sigma,epsilon,PllPoints,PlsPoints); % creates phi_l initial conditions
...
[P_1_inv] = P1MatrixFunction_CCC(Lambda,PlsPoints);
[P_2] = P2MatrixFunction_CCC(PlsPoints);
[C_1,D_star] = C1MatrixFunction_CCC(tStep,llStep,D_0,epsilon,PlsPoints,PllPoints);
...
[R_1] = R1MatrixFunction_CCC(Sigma_vec,Theta,e,epsilon,sigma,k_B,T,j_in,lsStep,RlsPoints,connection_type);
[A_1] = A1VectorFunction_CCC(Sigma_vec,var_rho,Upsilon,Theta,RlsPoints,connection_type);

% SOLVE FOR SOLID PHASE ELECTRIC CHARGE DENSITY AT NEXT TIME LEVEL

[rho_vec_Next,rho_net_vec_Next] = SolveRho_CCC(rho_vec_Current,R_1,A_1,var_rho,rho_t,connection_type);

% SOLVE FOR LIQUID PHASE IONIC CONCENTRATION AT NEXT TIME LEVEL

[c_vec_Next,c_net_vec_Next ] = SolveC_CCC(C_1,D_star,c_vec_Current,epsilon,c_0,rho_0,D_prime,D_0,F,rho_net_vec_Next,PlsPoints,PllPoints,lsStep);
 
 % SOLVE FOR SOLID PHASE ELECTRIC POTENTIAL AT NEXT TIME LEVEL

[phi_s_vec_Next] = SolvePhiS_CCC(rho_net_vec_Next,K,sigma,kappa_0,epsilon,PlsPoints);
 
 % SOLVE FOR LIQUID PHASE ELECTRIC POTENTIAL AT NEXT TIME LEVEL

[phi_l_vec_Next] = SolvePhiL_CCC(phi_s_vec_Next,kappa_0,sigma,epsilon,PlsPoints,PllPoints);
...
save CCCValidation.mat  rho_mat phi_s_mat phi_l_mat c_mat ls_mat ll_mat tVec vVecAbs tVec I_vec rho_net_vec_Current c_net_vec_Current phi_s_vec_Current phi_l_vec_Current

% (rho_mat(1,1)-rho_mat(end,1))/C_D/0.25/(1+sigma*(1-epsilon)/kappa_0)